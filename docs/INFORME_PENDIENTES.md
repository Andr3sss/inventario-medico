...
- Verificación actual del frontend y paquetes: lint, comprobación de tipos,
  compilación, 246 pruebas TypeScript y 3 pruebas Node de configuración de
  despliegue aprobadas. Los 2 escenarios E2E con
...
| ---------------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Usuarios, contraseña y roles                               | Parcial                 | PIN offline, cambio manual por Administrador con PIN, política fuerte y revocación están implementados; MFA quedó fuera del alcance aprobado. Falta aceptación en staging/producción. |
| Alta autónoma de usuarios por Administrador                | Cubierto técnicamente   | El Administrador asigna la contraseña inicial con confirmación por PIN; falta completar la prueba de aceptación en staging.                                                        |
...
> **Estado de implementación (13 de septiembre de 2026):** completado en el
...
> seguro. MFA fue retirado por decisión del alcance: el acceso central usa
> correo y contraseña. Desde el 15 de septiembre de 2026, el Administrador
> asigna y cambia contraseñas manualmente tras confirmar su PIN individual; el
> flujo público de recuperación y las invitaciones fueron eliminados. RLS y Edge Functions continúan
> comprobando perfil activo y roles. CORS es exacto, CI detecta secretos, la desactivación bloquea
...
> dominio productivo en este equipo. Por eso todavía deben
...
> filtradas, Site URL, secretos Edge, despliegue, Advisor y la
> matriz positiva/negativa. P0-07 no se declara cerrada hasta completar esa acta.

**Carencia:** la protección de contraseñas filtradas está deshabilitada y falta cerrar la validación productiva de cambio manual, revocación y secretos.

**Trabajo necesario:**

- Activar la protección contra contraseñas filtradas en Supabase Auth.
- Configurar la URL canónica del sitio.
- Validar reglas de contraseña, PIN administrativo, bloqueo y cambio manual.
